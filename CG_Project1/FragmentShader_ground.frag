...
uniform sampler2D groundTexture_diffuse;
...
vec3 CalcLightDirectional(LightDirectional light, vec3 uNormal, vec3 dirToCamera)
{
	...
	vec3 diffColor = 0.6 * diffIntensity * light.color * texture(groundTexture_diffuse, texCoord).rgb * vec3(1.0f, 0.92f, 0.8f);
	...
	vec3 specColor = 0.3 * specIntensity * light.color * texture(groundTexture_diffuse, texCoord).rgb;
	...
	//vec3 result = vec3(diffIntensity);
	return result;
};
...
vec3 CalcLightPoint(LightPoint light, vec3 uNormal, vec3 dirToCamera)
{
	...
	vec3 diffColor = diffIntensity * light.color * texture(groundTexture_diffuse, texCoord).rgb;
